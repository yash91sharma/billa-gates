...
export interface JobFormProps {
  job?: BackupJob
  /**
   * Handles the submitted payload. When it returns a promise the form stays
   * in a working state until it settles, so the seconds the backend spends
   * creating the repository are visible instead of looking like a dead page.
   */
...
export default function JobForm({
  ...
  // In-flight state. Creating a job is not a quick write: the backend checks
  // both mount sentinels and runs `restic init` on the destination drive
  // before it answers, which is seconds of silence after the click — long
  // enough to read as "nothing happened" and invite a second click that
  // would try to create the job twice.
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    // A submit is already in flight — the button is disabled, but Enter in a
    // text field submits the form too.
    ...
    try {
      ...
      // The caller owns error reporting — both call sites catch the API
      // failure themselves and render the message above the form. Swallowing
      // it here only avoids an unhandled rejection; the `finally` below is
      // what matters, so a failed create returns the form to a usable state
      // instead of leaving it stuck on "Creating…".
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <TooltipProvider>
      <form role="form" aria-label="Backup job form" onSubmit={handleSubmit} aria-busy={submitting}>
        {/* Everything sits in one fieldset so a submit in flight disables the
            whole form in a single place. The payload was captured when the
            request was sent, so anything typed while it is running would be
            silently discarded — the field must refuse the keystroke instead. */}
          ...
          <section>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                {destinationLabel && (
                  <p className="text-muted-foreground text-xs mt-1">
                    ⚠️ The root directory of destination <code>{destinationLabel}</code> must
                    contain a <code>.billa_gates_check</code> file.
                  </p>
                )}
                {isEdit && (
                  <>
            ...
            {retentionExpanded && (
              <div className="space-y-3 mt-3">
                ...
                <div className="bg-warning/15 border border-warning/40 rounded-sm p-3 text-xs mt-2">
                  ⚠️ <strong>Important:</strong> Restic retention policies (forgetting snapshots)
                  only remove snapshot metadata reference points. They <strong>do not</strong>{' '}
                  automatically free physical disk space. To reclaim physical space and prevent
                  silent disk accumulation, you must manually run a Prune operation from the Job
                  Details page.
                </div>
                {(
                  [
      ...
      {/* Working modal. The request is already on its way and the form is
          frozen behind it, so the dialog is deliberately undismissable: there
          is nothing to go back to until the backend answers. It is what makes
          the wait legible — the page dims, and the message names the step
          (repository setup) that accounts for the seconds. */}
        ...
        >
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {isEdit ? 'Saving changes…' : 'Creating job…'}
            </DialogTitle>
            <DialogDescription>
              {isEdit
                ? 'Applying the changes to this job and its schedule. This usually takes a moment — please keep this page open.'
                : 'Checking the source and destination are mounted, then setting up the restic repository on the destination drive. This can take a few seconds — please keep this page open.'}
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
      </Dialog>
